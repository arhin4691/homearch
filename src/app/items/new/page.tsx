...
export default function NewItemPage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      const payload = {
        ...data,
        expiryDate: data.hasExpiry && data.expiryDate ? new Date(data.expiryDate).toISOString() : undefined,
        bestBeforeDate: data.hasExpiry && data.bestBeforeDate ? new Date(data.bestBeforeDate).toISOString() : undefined,
      };
  ...
  return (
    <AppShell>
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-xl font-bold text-[var(--foreground)]">{t("addItem")}</h1>
      </div>
