...
export function ItemCard({ item, onFavoriteToggle, view = "grid" }: ItemCardProps) {
        ...
        <div className="flex-1 min-w-0">
          ...
          {item.location && (
            <div className="flex items-center gap-1 mt-1">
              ...
              <span className="text-xs text-[var(--muted)] truncate">{item.location.name}</span>
            </div>
          )}
        </div>
        {onFavoriteToggle && (
          ...
          >
            <Heart
              className={`h-5 w-5 transition-colors ${
                item.isFavorite ? "fill-red-500 text-red-500" : "text-[var(--muted)]"
              }`}
            />
          </button>
        )}
        ...
        <div className="flex items-center justify-between mt-1.5">
          ...
          {item.location && (
            <div className="flex items-center gap-0.5">
              <MapPin className="h-3 w-3 text-[var(--muted)]" />
              <span className="text-xs text-[var(--muted)] truncate max-w-[80px]">{item.location.name}</span>
            </div>
          )}
        </div>
      </div>
